		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			while (label) {
				...
				for (int Day = 1; Day <= 20; Day++) {
					...
					System.out.println(
							"Total amount player has after 20 days as per day included =" + totalAmount + " $");
					TotalAmountDiffInMonth = (TotalAmount - StartingStake) * 20;
					if (TotalAmountDiffInMonth > 0) {
						System.out.println(
								"Total cash gambler won in month" + month + "=" + TotalAmountDiffInMonth + "$");
					} else if (TotalAmountDiffInMonth < 0) {
						System.out.println(
								"Total cash gambler lost in month" + month + " =" + TotalAmountDiffInMonth + " $");
					} else {
						System.out.println("Total cash gambler neither lost or won in month" + month + "="
								+ TotalAmountDiffInMonth + "$");
					}
					System.out.println();
					if (TotalAmountDiffInMonth > 0) {
